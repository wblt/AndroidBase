...
//	    return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
	...
	public static String transformImage(String path){
		...
		if(fileS/1024 < 500){
			...
			try {
				outpath = FlowAPI.YYW_FILE_PATH + "comp" + filename;
				File f2 = new File(FlowAPI.YYW_FILE_PATH);
				if(!f2.exists()){
					f2.mkdirs();
				}
				...
				if(!file.exists()){
					if(file.createNewFile()){
						compressAndGenImage(getBitmap(path), outpath, 500);
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
	...
	public static String transformImage(String path, int size){
		File f1 = new File(path);
		long fileS = f1.length();
		String outpath = "";
		if(fileS/1024 < size){
			...
			try {
				...
				if(!file.exists()){
					if(file.createNewFile()){
						compressAndGenImage(getBitmap(path), outpath, size);
					}
				}
			} catch (IOException e) {
				e.printStackTrace();
